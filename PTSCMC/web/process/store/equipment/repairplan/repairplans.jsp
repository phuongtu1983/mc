...
<display:table style="width:100%" pagesize="15" requestURI="javascript:loadRepairPlanListSort({})" name="<%=Constants.REPAIRPLAN_LIST%>" id="repairplan" class="its" >
    ...
    <display:column >
        <div align="center"><input type="checkbox" id ="rpId" name="rpId" value="<%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getRpId()%>"></div>
    </display:column>
    ...
    <display:column titleKey="message.repairplan.timeRepair" sortable="true" headerClass="sortable">
        <a href="#" name="rpId" onclick="addRepairPlan(<%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getRpId()%>)"><%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getTimeRepair()%></a>
    </display:column>
    <display:column titleKey="message.repairplan.timeTrue" sortable="true" headerClass="sortable">
        <a href="#" name="rpId" onclick="addRepairPlan(<%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getRpId()%>)"><%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getTimeTrue()%></a>
    </display:column> 
    <display:column titleKey="message.repairplan.repairPart" sortable="true" headerClass="sortable">
        <a href="#" name="rpId" onclick="addRepairPlan(<%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getRpId()%>)"><%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getRepairPart()%></a>
    </display:column>     
    <display:column titleKey="message.repairplan.cost" sortable="true" headerClass="sortable">
        <a href="#" name="rpId" onclick="addRepairPlan(<%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getRpId()%>)"><%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getCost()%></a>
    </display:column> 
    <display:column titleKey="message.repairplan.repairKind" sortable="true" headerClass="sortable">
        <a href="#" name="rpId" onclick="addRepairPlan(<%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getRpId()%>)"><%=((RepairPlanBean)pageContext.getAttribute("repairplan")).getRepairKind()%></a>
    </display:column>
    <%}else{%>
