...
<div id="contractFormTitle"><h3><bean:message key="message.contractdetail.title"/>/<bean:message key="message.add.s"/></h3></div>
...
<form name="contractForm">
    <table width="100%">
        ...
        <tr>
            ...
            <logic:notPresent name="<%=Constants.TENDERPLAN_LIST%>">
                <td><bean:message key="message.contract.tender.number"/></td>
                <td colspan="3"><html:text size="30" property="tenderNumber" disabled="true" name="<%=Constants.CONTRACT%>"/></td>
            </tr>
            <tr>
                <td><bean:message key="message.contract.vendor"/></td>
        ...
        <tr>
            ...
            <td><html:text size="20" style="text-align: right" property="transport" name="<%=Constants.CONTRACT%>"  onblur="formatPositiveNumberMoney(this);caculateContract()" /></td>
            <td><bean:message key="message.contract.otherFee"/></td>
            <td><html:text size="20" style="text-align: right" property="otherFee" name="<%=Constants.CONTRACT%>"  onblur="formatPositiveNumberMoney(this);caculateContract()" /></td>
        </tr>
        <tr>
            ...
            <td colspan="4">
                <fieldset>
                    ...
                    <logic:equal name="<%=Constants.CONTRACT%>" property="isClosed" value="0">
                        <input type="button" onclick="return createDn(1, document.forms['contractForm'].conId.value);" value="<bean:message key="message.dn.add"/>"/>
                    </logic:equal>
                    <%}%>
                    <div>
                        <%
                    ...
                    <div id="listContractMaterialDataDiv" style="width:700px;height:300px;overflow:auto"><%@include  file="/process/purchasing/contract/details.jsp" %></div>
                </fieldset>
            </td>
        </tr>
    ...
    <logic:greaterThan name="<%=Constants.CONTRACT%>" value="0" property="conId">
        <img onclick="return printContract();" src="images/but_print.gif"/>
        <% if (formBean.getIsPermissionPrice() == 1) {%>
        <input value="<bean:message key="message.contract.paymentSum"/>" onclick="return printContractPaymentSum();" type="button">
        <%}%>
    </logic:greaterThan>
    <%
        if (PermissionUtil.hasPermission(request, PermissionUtil.FUNC_EDIT, PermissionUtil.PER_PURCHASING_CONTRACT, formBean.getOwnerId())) {
    %>
    ...
    <logic:equal name="<%=Constants.CONTRACT%>" property="isClosed" value="0">
        <!--<img onclick="return closeContract();" src="images/but_close.jpg"/>-->
    </logic:equal>
    <%}%>
    <html:image property="Back" value="Back" src="images/but_back.gif" onclick="return loadContractList();"/>
