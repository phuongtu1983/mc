                ...
                <fieldset>
                    ...
                    <table width="100%">
                        <tr>
                            ...
                            <td class="lbl10" style="display: none"><html:multibox property="approveSuggest" name="<%=Constants.REQUEST%>" value="4"></html:multibox> <bean:message key="message.request.approveSuggest3"/></td>
                            <td class="lbl10" style="display: none"><html:multibox property="approveSuggest" name="<%=Constants.REQUEST%>" value="8"></html:multibox> <bean:message key="message.request.approveSuggest4"/></td>
                            <td class="lbl10" style="display: none"><html:multibox property="approveSuggest" name="<%=Constants.REQUEST%>" value="16"></html:multibox> <bean:message key="message.request.approveSuggest5"/></td>
                        </tr>
                    </table></p>
                </fieldset>
            </td>
...
<tr>
    ...
    <logic:greaterThan name="<%=Constants.REQUEST%>" value="0" property="reqId">
        <logic:equal name="<%=Constants.REQUEST%>" value="0" property="highlight">
            <%if (form.getStoreApprove() == 1 && form.getIsMaterialNotCode()==0) {%>
            <input value="In VT" onclick="return printRequest(null,'VT');" type="submit">
            <!--<input value="In CCDC" onclick="return printRequest(null,'CCDC');" type="submit">-->
            <%}%>
        </logic:equal>
    </logic:greaterThan>
    <%

        //if (PermissionUtil.hasPermission(request, PermissionUtil.FUNC_EDIT, PermissionUtil.PER_PURCHASING_REQUEST, form.getCreatedEmp()) ||
        //PermissionUtil.hasPermission(request, PermissionUtil.FUNC_ADD, PermissionUtil.PER_PURCHASING_REQUEST, form.getCreatedEmp())) {
    %>
    <logic:present name="<%=Constants.CAN_SAVE%>">
        <html:image onclick="return saveRequest();" src="images/but_save.gif"/>
    </logic:present>
    <logic:present name="<%=Constants.CAN_EMAIL%>">
        <input value="<bean:message key="message.notify.request.notcode.email"/>" onclick="return emailForNotMaterialCode();" type="submit">
    </logic:present>
    ...
    <html:hidden property="isShortCut" name="<%=Constants.REQUEST%>"/>
    <logic:notPresent name="<%=Constants.CAN_EMAIL%>">
        <logic:greaterThan name="<%=Constants.REQUEST%>" value="0" property="reqId">
            <%
                if (form.getIsCreatedEmp() == 1) {
                    if (form.getStatus() == RequestBean.STATUS_STORE_ACCEPT && form.getIsMaterialNotCode()==0) {
            %>            
            <input type="button" onclick="return requestSentTo_PlanDep(<%=RequestBean.STATUS_MANAGER%>);" value="<bean:message key="message.request.changeToManage"/>"/>
            <%
                    }
                }
                if (form.getStatus() != RequestBean.STATUS_HANDLE  && form.getIsMaterialNotCode()==0) {
                    if (form.getPermission() == RequestBean.STATUS_CREATE) {
                        if (form.getStatus() == RequestBean.STATUS_CREATE) {
            %>
            <input type="button" onclick="return requestSentTo(<%=RequestBean.STATUS_STORE%>,null,emailRequestToStore);" value="<bean:message key="message.request.changeToStore"/>"/>
            <%}
            } else if (form.getPermission() == RequestBean.STATUS_STORE) {
                if (form.getStatus() == RequestBean.STATUS_STORE) {%>
            <!--<input type="button" onclick="return requestSentTo_Store(<%=RequestBean.STATUS_CHECK%>);" value="<bean:message key="message.request.changeToCheck"/>"/>-->
            <input type="button" onclick="return requestSentTo_Store(<%=RequestBean.STATUS_STORE_ACCEPT%>);" value="<bean:message key="message.request.changeToStoreAccept"/>"/>
            <%}
            } else if (form.getPermission() == RequestBean.STATUS_CHECK) {
                if (form.getStatus() != RequestBean.STATUS_PLANDEP) {%>
            <input type="button" onclick="return requestSentTo_Check(<%=RequestBean.STATUS_PLANDEP%>);" value="<bean:message key="message.request.changeToPlanDep"/>"/>
            <html:select property="purchaseOrg" name="<%=Constants.REQUEST%>" >
                <html:options collection="<%=Constants.ORG_OBJ%>" property="orgId" labelProperty="name"/>
            </html:select>
            <%}
            } else if (form.getPermission() == RequestBean.STATUS_PLANDEP) {
                if (form.getStatus() != RequestBean.STATUS_MANAGER && form.getIsMaterialNotCode()==0) {%>
            <input type="button" onclick="return requestSentTo_PlanDep(<%=RequestBean.STATUS_MANAGER%>);" value="<bean:message key="message.request.changeToManage"/>"/>
            <%}
            } else if (form.getPermission() == RequestBean.STATUS_MANAGER) {
                if (form.getStatus() != RequestBean.STATUS_HANDLE) {%>
            <input type="button" onclick="return requestSentTo_Manager(<%=RequestBean.STATUS_HANDLE%>);" value="<bean:message key="message.request.changeToHandle"/>"/>
            <%}
                    }
                }%>
            <%  if (form.getStatus() >= RequestBean.STATUS_HANDLE) {%>
            <input type="button" onclick="return copyRequest();" value="Copy"/>
            <%}%>
        </logic:greaterThan>
