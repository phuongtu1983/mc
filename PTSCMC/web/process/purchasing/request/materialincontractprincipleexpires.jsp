...
<display:table style="width:100%" pagesize="10" requestURI="javascript:loadMaterialInContractPrincipleExpires({})" name="<%=Constants.MATERIAL_PRINCIPLE_LIST%>" id="mat" class="its" >
    ...
    <display:column titleKey="message.empty">
        <input type="checkbox" name="matId" value="<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getMatId()%>" id="reqDetId_materialInContractPrincipleExpireForm_<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getReqDetailId()%>_<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getConId()%>" onclick="return materialInContract_CheckOnClick('materialInContractPrincipleExpireForm','<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getReqDetailId()%>_<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getConId()%>',<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getMatId()%>,<%=((MaterialInContractFormBean)pageContext.getAttribute("mat")).getReqId()%>,<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getConId()%>,<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getVenId()%>,<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getConDetId()%>)"
    ...
    <display:column titleKey="message.empty">
        <input type="button" onclick="return cancelRequestRemainQuantity(<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getReqDetailId()%>,loadMaterialInContractPrincipleExpires)" value='<bean:message key="message.cancel"/>'/>
    </display:column>
</display:table>
<br/>