...
<display:table style="width:100%" pagesize="10" requestURI="javascript:loadMaterialInOrders({})" name="<%=Constants.MATERIAL_CONTRACT_LIST%>" id="mat" class="its" >
    ...
    <display:column style="" titleKey="message.contract.number" sortable="true" headerClass="sortable">
        <a href="#" onclick="return printOrder(<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getConId()%>);"><%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getConNumber()%></a>
    </display:column>
    ...
    <display:column titleKey="message.empty">
        <input type="button" onclick="return cancelRequestRemainQuantity(<%=((MaterialInContractFormBean) pageContext.getAttribute("mat")).getReqDetailId()%>,loadMaterialInOrders)" value='<bean:message key="message.cancel"/>'/>
    </display:column>
</display:table>
