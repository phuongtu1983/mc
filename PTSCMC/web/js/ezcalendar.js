...
==============================================================================*/
	...
	function updateCalendar() {
		...
		m = m.value;
                 if(target!=null){
                    dd=getID(target).value.substring(0,2).toString();
                    mm=getID(target).value.substring(3,5).toString();
                    yyyy=getID(target).value.substring(6,10).toString();
                    if(dd.substring(0, 1)=="0") dd=getID(target).value.substring(1,2).toString();
                    if(mm.substring(0, 1)=="0") mm=getID(target).value.substring(4,5).toString();
                 }
				...
				for (c=0;c<7;c++) {
					...
					if (calendarDate.getDate()+"/"+calendarDate.getMonth()+"/"+calendarDate.getFullYear()==today.getDate()+"/"+today.getMonth()+"/"+today.getFullYear()) {
						getID("cell"+r+c).className="today";
                                        ...
                                        if(target!=null){
                                            // was it selected date?
                                            if (calendarDate.getDate()+"/"+(calendarDate.getMonth()+1)+"/"+calendarDate.getFullYear()==dd+"/"+mm+"/"+yyyy) getID("cell"+r+c).className="selectday";
                                        }
	...
	function setDateValue(d,el) {
		if (bCalendarFade) {
			...
			getID("EZcalendar").style.display = "none";
                         //phuongtu
                         if(target!=null) getID(target).focus();
		}
	...
	//document.onload = setTimeout("initCalendar()",500); //phuongtu
