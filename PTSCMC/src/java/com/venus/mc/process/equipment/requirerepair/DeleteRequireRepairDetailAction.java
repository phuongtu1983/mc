 ...
 * @author mai vinh loc
 */
public class DeleteRequireRepairDetailAction extends SpineAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        RequireRepairDetailBean bean = null;
        try {
            ...
            for (int i = 0; i < length; i++) {
                if (!arrDetId[i].equals("0")) {                    
                    bean = requestDAO.getRequireRepairDetail(Integer.parseInt(arrDetId[i]));
                    requestDAO.deleteRepairNotPlan(bean.getEquId());
                    requestDAO.deleteRequireRepairDetail(arrDetId[i]);
                }
            }
