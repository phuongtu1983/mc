 ...
 * @author mai vinh loc
 */
public class DeleteAcceptanceTestDetailAction extends SpineAction {
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
                if (!arrDetId[i].equals("0")) {
                    requestDAO.deleteAcceptanceTestDetail(arrDetId[i]);
                }
            }
            ...
            LogUtil.error("FAILED:AcceptanceTestDetail:delete-" + ex.getMessage());
            ex.printStackTrace();
        }
        return true;
    }
