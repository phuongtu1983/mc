...
public class MaterialUsedFormAction
  extends SpineAction
{
  public boolean doAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
  {
    ...
    if (!GenericValidator.isBlankOrNull(umsId)) {
      try
      {
        formBean = storeDAO.getUsedMaterialStore(Integer.parseInt(umsId));
    ...
    if (formBean == null) {
      try
      {
        ...
        else
        {
          ...
          formBean = new UsedMaterialStore2FormBean();
          ...
          String number = storeDAO.getNextUSMNumber(prefix, 4);
          prefix = prefix + number;
          formBean.setUsmNumber(prefix);
        }
      }
    ...
    request.setAttribute("usedMaterialStore", formBean);
    if (storeList == null) {
      storeList = new ArrayList();
    }
    request.setAttribute("umsDetailList", storeList);
