 ...
 * @author phuongtu
 */
public class DeleteVendorEvalAction extends SpineAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        String[] arrEvalid = request.getParameterValues("venevalid");
        try {
            ...
            if (arrEvalid != null) {
                length = arrEvalid.length;
            }
            for (int i = 0; i < length; i++) {
                vendorDAO.deleteVendorEval(arrEvalid[i]);
            }
            OutputUtil.sendStringToOutput(response, "deleted");
        } catch (Exception ex) {
            LogUtil.error("FAILED:VenderEval:delete-" + ex.getMessage());
            ex.printStackTrace();
        }
        return true;
    }
