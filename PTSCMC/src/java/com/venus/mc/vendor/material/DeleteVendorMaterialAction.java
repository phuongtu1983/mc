 ...
 * @author phuongtu
 */
public class DeleteVendorMaterialAction extends SpineAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        String[] arrMaterialid = request.getParameterValues("venmaterialid");
        try {
            ...
            for (int i = 0; i < length; i++) {
                vendorDAO.deleteVendorMaterial(arrMaterialid[i]);
            }
            OutputUtil.sendStringToOutput(response, "deleted");
        } catch (Exception ex) {
            LogUtil.error("FAILED:VenderMaterial:delete-" + ex.getMessage());
            ex.printStackTrace();
        }
        return true;
    }
