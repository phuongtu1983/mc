 ...
 * @version
 */
public class ListVendorGroupMaterialAction extends SpineAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        if (!GenericValidator.isBlankOrNull(venId)) {
            try {
                vendorGroupMaterialList = vendorDAO.getVendorGroupMaterials(Integer.parseInt(venId));
            } catch (Exception ex) {
                LogUtil.error("FAILED:VenderGroupMaterial:list-" + ex.getMessage());
                ex.printStackTrace();
            }
        }
        request.setAttribute(Constants.VENDOR_GROUP_MATERIAL_LIST, vendorGroupMaterialList);
        return true;
    }
