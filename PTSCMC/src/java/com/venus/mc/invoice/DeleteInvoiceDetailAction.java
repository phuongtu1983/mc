 ...
 * @author phuongtu
 */
public class DeleteInvoiceDetailAction extends SpineAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
                contractDAO.deleteInvoiceDetail(NumberUtil.parseInt(arrDetId[i], 0));
            }
            OutputUtil.sendStringToOutput(response, "deleted");
        } catch (Exception ex) {
            LogUtil.error("FAILED:InvoiceDetail:delete-" + ex.getMessage());
            ex.printStackTrace();
        }
        return true;
    }
