...
public class CopyRequestFormAction
  extends SpineAction
{
  public boolean doAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
  {
    ...
    if (reqId != 0)
    {
      ...
      try
      {
        reqBean = requestDAO.getRequest(reqId);
        if (reqBean != null)
        {
          formBean = new RequestFormBean(reqBean);
          formBean.setRequestNumber(formBean.getRequestNumber() + "-" + requestDAO.getNextRequestNumber(formBean.getRequestNumber(), 4));
          
          OrganizationDAO orgDAO = new OrganizationDAO();
          String orgIds = MCUtil.getOrganizationID(request.getSession()) + "";
          orgIds = orgIds + "," + orgDAO.getnestedParentOfOrg(orgIds);
          formBean.setAssignedEmp(requestDAO.getRequestAssignedEmp(reqId, orgIds));
          String matIds = requestDAO.getMaterialIdNotCode(reqId);
    ...
    if (reqId > 0) {
      if (formBean.getAssignedEmp() == MCUtil.getMemberID(request.getSession())) {
        ...
        try
        {
          if (reqDAO.isAssignRequestOfRequest(MCUtil.getMemberID(request.getSession()), reqId))
          {
            formBean.setIsAssignEmp(1);
    ...
    if (reqId != 0)
    {
      if (formBean.getCreatedEmp() == MCUtil.getMemberID(session))
      {
        ...
        try
        {
          ...
          if (reqDAO.isAssignRequestPosition(MCUtil.getMemberID(session), reqId))
          {
            request.setAttribute("canSave", "true");
    ...
    if (reqId > 0) {
      try
      {
        String str = reqDAO.getMaterialNullCode(reqId);
        if (str.length() > 0) {
