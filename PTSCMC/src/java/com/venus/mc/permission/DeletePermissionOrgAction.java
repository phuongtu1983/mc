 ...
 * @author phuongtu
 */
public class DeletePermissionOrgAction extends SpineAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        String[] arrDetId = request.getParameterValues("permissionOrgChk");
        try {
            ...
            String[] arrOrg = bean.getOrganizations().split(",");
            String orgs = "0";
            for (int i = 0; i < arrOrg.length; i++) {
                boolean existed = false;
                for (int j = 0; j < length; j++) {
                    if (arrOrg[i].equals(arrDetId[j])) {
                        existed = true;
                        break;
                    }
                }
            ...
            if (!orgs.equals("0")) {
                bean.setOrganizations(orgs.substring(2));
                try {
            ...
            LogUtil.error("FAILED:PermissionOrg:delete-" + ex.getMessage());
            ex.printStackTrace();
        }
        return true;
    }
