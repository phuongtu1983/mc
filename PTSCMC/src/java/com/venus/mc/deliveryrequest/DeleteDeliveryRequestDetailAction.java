...
public class DeleteDeliveryRequestDetailAction
  extends SpineAction
{
  public boolean doAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
  {
    ...
    try
    {
      ...
      {
        for (int i = 0; i < length; i++) {
          contractDAO.deleteDeliveryRequestDetail(NumberUtil.parseInt(arrDetId[i], 0));
        }
    ...
    {
      LogUtil.error("FAILED:DeliveryRequestDetail:delete-" + ex.getMessage());
      ex.printStackTrace();
    }
    return true;
  }
