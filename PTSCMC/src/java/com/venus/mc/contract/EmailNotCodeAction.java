    ...
    public boolean doAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
  {
    ...
    if (!GenericValidator.isBlankOrNull(conId)) {
      try
      {
        ...
        int kind = contractDAO.getKindfromConId(conId);
        if (kind == 8) {
          matIds = contractDAO.getMaterialForEmailAdjustment(NumberUtil.parseInt(conId, 0));
        } else {
          matIds = contractDAO.getMaterialForEmail(NumberUtil.parseInt(conId, 0));
        }
        if (!matIds.equals("0"))
        {
          notifyForNotMaterialCode(matIds, NumberUtil.parseInt(conId, 0), MCUtil.getMemberID(request.getSession()));
    ...
    private void notifyForNotMaterialCode(String matIds, int conId, int empId) {
        try {
            ContractDAO contractDAO = new ContractDAO();
            ContractBean conBean = contractDAO.getContract(conId);
            MaterialDAO matDAO = new MaterialDAO();
            ...
            if (conBean != null) {
                ...
                content += "<table align='center' width='70%'><tr><td class='cart_b' width='761' height='31' align='center'>"
                        ...
                        + MCUtil.getBundleString("message.notify.contract.notcode.title") + " " + conBean.getContractNumber()
                        + "</b></span></td></tr><tr><td>";
                ...
                content += " đơn hàng số " + conBean.getContractNumber() + " ngày " + conBean.getSignDate()
        ...
        String mail_subject = MCUtil.getBundleString("message.notify.contract.notcode.title") + " " + conBean.getContractNumber();
